...
const api: OrbitApi = {
  ...
  net: {
    ...
    onPeers: (cb) => {
      ...
      return () => ipcRenderer.removeListener('net:peers', listener);
    },
    onInvite: (cb) => {
      const listener = (_e: unknown, invite: OrbitInvite) => cb(invite);
      ipcRenderer.on('net:invite', listener);
      return () => ipcRenderer.removeListener('net:invite', listener);
    },
  },
  friends: {
    list: () => ipcRenderer.invoke('friend:list'),
    add: (friend) => ipcRenderer.invoke('friend:add', friend),
    forget: (id) => ipcRenderer.invoke('friend:forget', id),
  },
  midi: {
