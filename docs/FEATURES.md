...
| **Auto-mapa por el nombre del archivo**: se sueltan las muestras encima y entran con su nota (`Piano_C3.wav`, `Gtr-A#2.wav`, `EP C4 v3.wav`), repartiéndose los rangos solas. Se toma en serio los falsos positivos —`Bass2.wav` no es un si— y lo que no sabe leer lo dice en vez de colocarlo a bulto: una nota en el sitio equivocado no se ve, se descubre tocando. Desplazamiento de octavas para librerías con otra convención | v3.1 |
