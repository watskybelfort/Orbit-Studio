...
| **`npm run listen:kit`**: el material que hay que ESCUCHAR, renderizado de los mismos fixtures del golden — así lo que se oye es exactamente lo que el banco fija. Colas de 25 s (el golden corta a 1-3 para caber en la CI, y un piso de continua no se oye en dos segundos) y 24 bits (a 16 el contenedor esconde lo que se busca), más un índice que dice qué buscar en cada archivo y la referencia de libopus al mismo bitrate. No renderiza el «antes» de un cambio de DSP: para eso tendría que editar `packages/engine/src`, y eso un comando no lo hace | v3.9 |
