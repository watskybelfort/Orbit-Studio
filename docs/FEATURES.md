...
| **Controladores de verdad**: cada dispositivo se enciende y se apaga por su cuenta, filtro de canal (omni o uno), transposición por octavas y curva de pulsación (suave/lineal/dura/fija). Panel en Ajustes con LED de actividad — cuando un teclado "no funciona", lo primero es saber si sus mensajes llegan | v3.0 |
| **Pedal de sostenido** (CC 64) por dispositivo, con histéresis en la mitad de la carrera (hay pedales continuos). Resuelve los dos casos que dejan notas colgando: repicar una tecla con el pedal pisado y apagar el teclado sin levantarlo | v3.0 |
| **Rueda de tono que dobla el tono**: reafina la voz VIVA en los diez instrumentos, cada uno con su moneda (frecuencia los sintes, ritmo de lectura el sampler, la fundamental de la que cuelgan las capas en Nova y Prisma, y en el kit la parte tonal *y* el filtro). El kernel guarda los semitonos por canal, así que una nota tocada con la rueda sujeta nace ya doblada en vez de trepar durante el ataque. Rango elegible (±1 a ±24) y zona muerta en el centro, porque una rueda con muelle no vuelve nunca al valor exacto. Si la rueda está aprendida a un destino, manda el destino | v3.2 |
| **La rueda, grabada**: deja de ser un mensaje suelto al motor y pasa a ser un parámetro del canal (`bend`, en semitonos, ±24). Con eso llegan la curva de automatización —dibujable a mano en el editor, con sus puntos en semitonos y con signo—, el LFO (un vibrato sobre la rueda sale gratis), el undo, el viaje a la sala y la grabación al mover la rueda tocando. La rueda sigue yendo al motor directa en cada mensaje, porque es un gesto y entre moverla y oírla no puede haber ni un frame; al proyecto se vuelca una vez por frame y con `mergeKey`, así que un barrido entero es UN paso de undo. Un canal que no declara `bend` significa "no tengo opinión" y no "está centrada": si significara centro, recompilar el proyecto soltaría la rueda de golpe en mitad del gesto | v3.4 |
| **MIDI learn**: clic derecho en cualquier perilla o fader → Aprender MIDI, y el mando queda atado a ese parámetro. Escribe por el bus de comandos (con undo, y viaja a la sala), a un volcado por frame y con un solo paso de undo por barrido. El mapa va por mando (`cc:74`), no por dispositivo: cambiar de puerto USB no lo rompe | v3.0 |
| **Grabación MIDI con el tiempo del evento**: la nota cae donde estaba el playhead cuando la tocaste, no donde estaba cuando la UI se enteró (hasta 46 ms de diferencia). Rejilla elegible (sin cuantizar, 1/32, 1/16, 1/8, negra) y volcado al patrón al cerrar CADA vuelta del loop —overdub— partiendo las teclas que sigas pulsando | v3.0 |
...
| **Analizador de espectro por pista**, en el panel de cadena del mixer y sin abrir ningún EQ: para ver dónde choca el 808 con el bombo sin exportar. FFT de 1024 con Hann y suavizado, todo preasignado, corriendo en la UI y no en el audio thread | v3.5 |
| **Medidor de LUFS en vivo** en el strip del Master (integrado, short-term y true peak), para saber que vas a −14 ANTES de exportar. Réplica incremental del BS.1770 con el mismo gateado que el análisis offline, y coincide con él dentro de 3×10⁻⁹ LU sobre el mismo material — si no coincidieran, el que está mal es el de en vivo | v3.5 |
| **Guarda contra ciclos de enrutado**: rutar una pista a otra que ya vuelve a ella dejaba la mezcla entera en silencio digital (−240 dBFS) sin error, sin aviso y sin marca — un clic en «Enrutar aquí» y el DAW dejaba de sonar sin decir por qué. Ahora `setRoute` lo rechaza antes de mutar, reusando el MISMO detector que el editor de nodos ya usaba para bloquear el arrastre, y el menú del mixer enseña el motivo en el banner. Se mantiene la ruta anterior en vez de romper el otro extremo por tu cuenta | v3.6 |
| **La pestaña de mezcla dice la verdad** sobre por dónde sale un canal: leía el campo crudo, así que a un canal de una carpeta con bus le decía «Sale por Master» cuando en realidad sonaba por el bus | v3.6 |
...
| **Pack Instrumentos** | 24 instrumentos con altura por síntesis (pianos, EPs, guitarras Karplus-Strong, bajos, órganos, pads, campanas, leads — 3-4 variantes por familia) | v0.5 |
| **Pack Instrumentos en multisample** | Cada uno grabado a TRES alturas —su registro natural y una octava a cada lado— y montado como keymap al soltarlo: se acabó la muestra estirada por cinco octavas. Con la altura como parámetro salieron las cuatro cosas que solo se ven grabando en dos octavas: la semilla del ruido lleva la altura (si no, las tres tomas de un pad respiran enganchadas), los filtros de los sintéticos siguen a la nota (los de los acústicos no: el martillo y el soplo son formantes), la cuerda pulsada se afina con retardo fraccionario, y el tine 14:1 del EP deja de doblarse sobre el Nyquist arriba | v3.2 |
| **Capas de fuerza en el pack** | Cada una de las tres alturas se graba además con dos pulsaciones, porque tocar flojo no da el mismo instrumento más bajito: da otro timbre. Cada familia responde con su moneda —en un piano acústico se empina la caída espectral y se apaga el martillo, en uno eléctrico la pulsación ES el índice de FM, en una cuerda pulsada es lo romo de la excitación, en un bajo cuánto se abre la envolvente de filtro, en una campana el badajo— y en un órgano casi nada, que es la respuesta correcta: un Hammond no responde a la pulsación y solo ceden el click de los contactos, la percusión y los drawbars de arriba. La capa fuerte es la grabación del pack anterior BIT A BIT, así que ningún proyecto guardado cambia de sonido. La pulsación no entra en la semilla del ruido —dos capas de una nota son la misma cuerda golpeada distinto— y de ahí la regla: cambia amplitudes, índices y cortes, nunca la estructura de los bucles | v3.3 |
| **Una tercera capa de fuerza** | Con dos, el salto de flojo a fuerte caía en un punto del recorrido y se podía oír si se buscaba; con tres hay dos escalones más pequeños. `DYNAMICS` pasa de [0.35, 1] a [0.35, 0.675, 1] y los sufijos a `-pp` / `-p` / sin sufijo, o sea que **la capa floja de siempre no se recalculó: se renombró**. Las dos capas previas sobreviven BIT A BIT (72/72 cada una por sha256) y lo único audio nuevo son las 72 de en medio — que funcione así no es suerte, es la consecuencia de que la pulsación no entre en la semilla del PRNG. El pack pasa de 71 a 98,87 MB y de 205 a 277 archivos | v3.5 |
| **Multisample (keymaps)** | Un canal de sampler con varias muestras repartidas por el teclado y por velocidad, cada una sonando a su nota real. Una zona es un rectángulo (tecla × velocidad) con su muestra, su raíz, su afinación fina y su ganancia; las zonas pueden solaparse a propósito —así se hacen las capas y así se apilan dos micros de la misma toma— y todas las que caigan bajo la nota suenan. Editor con la tira del teclado (las capas se ven apiladas en altura) y la lista con los números exactos | v3.1 |
| **Auto-mapa por el nombre del archivo** | Se sueltan las muestras encima y entran con su nota (`Piano_C3.wav`, `Gtr-A#2.wav`, `EP C4 v3.wav`), repartiéndose los rangos solas. Se toma en serio los falsos positivos —`Bass2.wav` no es un si, `take01/02/03` no son las teclas 1, 2 y 3, y la nota NO se busca en el nombre de la carpeta— y lo que no sabe leer lo dice en vez de colocarlo a bulto. Desplazamiento de octavas para librerías con otra convención. Los instrumentos del pack no pasan por aquí: traen su nota escrita | v3.1 |
...
| Contenido de fábrica generado por síntesis propia (pack Orbit Essentials: 84 sonidos en 204 grabaciones, 70 MB — cifras del v0.1; hoy son 276 grabaciones y 98,87 MB, ver la fila de la v3.5) | v0.1 |
...
| **Entradas de más de dos canales**: la entrada del kernel era estéreo fija, así que con una interfaz de 8 te quedabas con el par que pusiera el sistema. Ahora una **ruta de entrada** (canal L, canal R opcional, pista, armado, monitor, ganancia) dice qué entrada física va a qué pista, y se pueden grabar varias a la vez desde UN stream multicanal. Sin rutas declaradas sale la implícita —el par 1-2—, así que un proyecto viejo graba igual | v3.5 |
| **Compensación de latencia de la toma**: entre el micro y el kernel está el buffer del sistema, y el clip caía unos milisegundos tarde respecto de lo que se oía al cantar. Se MIDE con un chirp por la salida y correlación cruzada de la captura. Lo importante es que **no mide mal**: puerta de RMS antes de correlacionar (con auriculares la salida no llega, y correlacionar ruido contra ruido siempre da un máximo), pico mínimo y margen sobre el segundo candidato. Un rechazo nunca pisa el valor guardado | v3.5 |
| **No se puede cambiar de dispositivo ni cerrar el micro con una toma en curso**: hacerlo cerraba el stream y la captura dejaba de acumular muestras **sin excepción ni aviso** — el clip salía corto y desincronizado, y no te enterabas hasta escucharlo. Ahora los dos controles se bloquean mientras grabas, con el motivo a la vista | v3.6 |
| **La confianza de la calibración de latencia se guarda**: al reiniciar decía «Sin calibrar todavía» mientras el retardo medido, dos líneas más abajo, ya se estaba aplicando | v3.6 |
| **Ganancia por ruta de entrada**: un deslizador con la lectura en dB junto a cada ruta. El modelo, el comando y el kernel ya la aplicaban desde la v3.5 — lo único que faltaba era el mando | v3.7 |
| **La guarda del micro vive en las funciones, no en el botón**: cambiar de dispositivo o cerrar el micro en pleno REC truncaba la toma en silencio. La v3.6 lo tapó bloqueando los controles en `InputSection.tsx`, así que el primer atajo de teclado, paleta de comandos o acción de MCP que alguien añadiera resucitaba el bug entero. Ahora la regla está en `toggleInputListening`/`setInputDevice` —donde no se puede esquivar— y la UI **lee** el motivo con `useInputGuardReason()` en vez de repetirlo: dos copias de una invariante se desincronizan sin que ningún test lo note | v3.8 |
| **Desconectar la interfaz a mitad de una toma ya avisa**: el hot-unplug (`track.onended`) y el cambio de dispositivo por defecto del sistema (`ondevicechange`) no estaban manejados, así que la captura se quedaba muda y el clip salía corto sin que te enteraras hasta escucharlo. No se puede bloquear —el cable ya se fue—, así que se para la grabación y se dice que la toma se cortó ahí, para que quien cantó sepa que hay que repetirla | v3.8 |
...
| Opus propio — **HECHO**. Códec entero escrito en casa (range coder, MDCT, PVQ, energía, asignador, contenedor), con la RFC 6716 como referencia. Verificado contra ffmpeg: 12 configuraciones con correlación 0,997-1,000, retardo cero y ganancia 1,000 (`tools/qa/opus-verify.ts`). El export ya ofrece `.opus` de 64 a 256 kbps. Falta afinar decisiones (postfiltro, transitorios, dispersión adaptativa) para acercarse a libopus en calidad por bit | v2.8 |
| **La inclinación del reparto ya no es fija**: `alloc_trim` sale de la pendiente del espectro (momento de primer orden, portado de `alloc_trim_analysis`) en vez de estar clavada en neutro. Con el 5 fijo, un acorde recibía el mismo esfuerzo en las bandas vacías que en las que llevaban la música. Y hay con qué medirlo: `tools/qa/opus-quality.ts` codifica la misma señal con Orbit y con libopus al mismo bitrate y compara las dos con el original — la distancia media bajó de −2,06 a −1,21 dB. La dispersión adaptativa se implementó y no entró: la SNR no ve lo que hace, salió neutra, y sin poder demostrar la mejora no se sube | v3.4 |
| Export MIDI multipista junto al WAV (flujo FL de Orbit: .mid + wav) | v0.2 |
| **El export de stems deja de clonar el proyecto por pista**: `renderOpts.samples` lleva el audio entero y se pedía un stem por petición, o sea una copia completa de cada uno. Ahora van por lotes de 4 — medido, 88,5 ms de clones pasan a ~24 — y no todos juntos, porque eso deja 649 MB de RSS vivos contra 132. El progreso llega desde el worker, casado por id, y el watchdog se rearma en cada mensaje | v3.5 |
| **El encoder Opus, medido con oído**: el banco de calidad tiene ahora una medida perceptual (PEAQ simplificado sobre el modelo de la BS.1387) además de la SNR, y **para decidir manda ella**. Con eso entró la dispersión adaptativa —que llevaba fuera por medirse neutra en SNR— y se vio que el agujero grande eran los transitorios. La distancia media a libopus pasa de −3,59 a **−1,52 dB** de patrón | v3.5 |
| **Detección de transitorios**: la trama se parte en 8 MDCT de 2,5 ms cuando hay un ataque, y `tf_analysis` decide banda a banda cuánta resolución temporal se queda. El pre-eco de un click aislado baja de −12,6 a **−32,5 dB**, por delante de libopus (−31,6) | v3.5 |
| **Y un bug de sincronía que arrastraba desde el primer día**: el encoder marcaba la trama como silencio y seguía escribiendo, mientras el decodificador —al leer esa bandera— descartaba el resto y dejaba todas las bandas en silencio. Los dos lados predecían desde sitios distintos y el desfase se amplificaba banda a banda. Se oía en cada golpe del pack de batería, que es donde hay silencio digital entre golpes | v3.5 |
| **Un stem que falla ya no se lleva a los hermanos de su lote**: como los stems se piden de cuatro en cuatro, una excepción tiraba el lote entero y perdía hasta tres ya renderizados con éxito. Ahora cada pista va aislada y el export SIGUE — escribe los buenos y avisa de cuál faltó, con su motivo | v3.6 |
| **Cancelar un export a medias**: doce stems tardan minutos y hasta ahora solo se podía esperar o matar la app, lo que deja archivos a medio escribir. Botón con el estado a la vista, y la cancelación atendida en tres capas: entre lotes, entre pistas del lote y **dentro** del render de una pista, que era el punto que faltaba (una sola pista larga ya tarda). El checkpoint del bucle caliente no añade una comparación al caso común —cuelga del mismo `if` que ya pagaba el progreso, medido en −3,79 %, dentro del ruido— y en la app el corte real es un `terminate()` del worker. Lo ya escrito se conserva, mismo criterio que la v3.6 para el stem que revienta; y nada queda a medias porque el corte se lee ANTES de empezar cada archivo, nunca contra uno en marcha | v3.8 |
...
| **Plantillas de proyecto**, en el Browser y en el menú Archivo: las cuatro de fábrica (trap con 808, boom bap, reggaetón y voz sobre beat) y **las tuyas**, guardadas con «Guardar el proyecto actual como plantilla…». Una plantilla es un `.orbit` normal, sin formato nuevo; las de fábrica se generan en el momento desde `PROJECT_TEMPLATES` de core, así que hay UNA sola fuente y el menú y el Browser no pueden divergir | v3.5 |
| **Aviso de versión nueva**: mira si hay una release más reciente y enseña un cartel discreto con el enlace. No es un autoUpdater: no descarga ni reinicia nada. La consulta la hace el proceso principal porque **el renderer no tiene red** (la CSP se lo prohíbe: ahí corre código que no es nuestro), y el canal IPC no acepta argumentos — la URL es fija, así que no abre superficie nueva. Falla en silencio sin red y se apaga desde Ajustes | v3.5 |
...
| **Un plugin puede pintar su propia vista** (`createView`): una curva de compresión, la respuesta de un EQ, las repeticiones de un delay. La frontera es **de datos, no de DOM** — lo único que el plugin toca del host es un grabador que escribe floats en un array, porque con un contexto de canvas real `ctx.canvas.ownerDocument` daría `document` y con él `window.orbit`. El `draw` corre en un Worker propio que se desarma (`fetch`, `XHR`, `WebSocket`, `navigator`… en `undefined` no reconfigurable) **antes** de compilar nada, los colores son índices a la paleta del tema, y un opcode desconocido corta el repintado. Un plugin que se cuelga pintando se mata a los 500 ms y **el efecto sigue sonando** | v3.5 |
| **Un instrumento pinta su propia interfaz en el Channel Rack**, no solo los efectos en el mixer: botón ⧉ que abre su vista en un popover. Por dentro delega en el mismo `PluginView` y el mismo worker aislado, así que no hay un segundo camino de dibujo que pueda divergir del primero | v3.7 |
...
| Tests unitarios del DSP con render determinista — 778 tests en total. (No son los «golden tests»: ese nombre lo reserva el repo para `engine/test/golden`, que nació en la v3.7 — ver más abajo) | v0.1 |
...
| **El worklet suelta los samples que ya no usa nadie**. Su mapa se escribía y no se borraba NUNCA: una sesión larga acumulaba en el hilo de audio cada byte que pasó por delante. Se cuentan siete fuentes de referencia (canal, cortes del slicer, keymap, clips, registro, pins y las raíces del kernel) **sobre el proyecto editable y no sobre el compilado** —contar ahí soltaría el audio de un clip por tenerlo muteado—, y un sample con una voz viva encima espera a que calle. Abrir otro proyecto libera el audio del anterior | v3.5 |
| **La reverb deja de gastar 34× en silencio**: al decaer sin llegar a cero el estado entraba en rango denormal y cada operación costaba un orden de magnitud más — con la CPU subiendo justo cuando la música PARA. Medido: de ~65 ns/muestra a 2000–2400 tras ~140 s de cola; con el flush, plano en 79–97 durante 12M muestras | v3.5 |
| **Los filtros dejan de escalonar al automatizar**: los coeficientes se deslizan en 5 ms en vez de aplicarse de golpe (el zipper noise sobre un barrido). Cada clase interpola donde no puede romperse — el SVF en g/k del TPT, y el biquad en coeficientes crudos, que es seguro porque su región de estabilidad es un triángulo, y un triángulo es convexo | v3.5 |
| **Cero alocaciones en el audio thread, ahora vigilado**: un test sustituye `globalThis.Float32Array` por una subclase que cuenta y afirma 0 en 32 bloques con cuatro entradas grabando a la vez. Antes era disciplina humana | v3.5 |
| **Los samples se sueltan mientras trabajas**, no solo al cambiar de proyecto. Antes la recolección únicamente corría al reemplazar el proyecto entero — y aunque hubiera corrido no habría liberado nada, porque un sample cuenta como vivo mientras siga registrado y nadie lo desregistraba. Ahora se pregunta si algún undo, redo o rama archivada puede volver a nombrarlo: el que acabas de borrar sigue vivo en el inverso de ese cambio y se conserva; el que ya cayó del historial se suelta. Cuando duda, conserva | v3.6 |
| **Denormales fuera de todos los lazos con realimentación**, no solo de la reverb: Biquad, SVF y Allpass1 en `filters.ts`, y delay, flanger y phaser. Importa porque Biquad es el EQ que corre en cada canal del mixer: un filtro resonante tras el silencio entraba en un ciclo límite permanente en rango subnormal —confirmado a 8 millones de muestras— a 73 ns/muestra contra 9-12 de referencia. El piso que se introduce queda entre 4 y 11 órdenes de magnitud por debajo del de 24 bits | v3.6 |
| **Golden tests de sonido** (`packages/engine/test/golden`): 24 renders deterministas y 2 flujos Opus que fijan el timbre. Dos capas a propósito — el hash sha256 dice QUE algo cambió, y una matriz de medidas perceptuales (LUFS, peak, 4 bandas, correlación; del total y de 8 ventanas, 0,01 dB de tolerancia) dice QUÉ cambió. El hash se compara sin condicional por plataforma, y eso está medido con Docker: el mismo bundle sale bit a bit idéntico en x64 aguantando cambio de SO y tres versiones mayores de V8. Actualizar la línea base pide `npm run golden:update --accept "<motivo>"`; nunca hay un `--update-snapshots` | v3.7 |
| **La caché de samples del render deja de crecer**: vivía en el hilo de la UI y no la vaciaba nadie. Cinco proyectos de dieciséis tomas largas acumulaban 6460 MiB; ahora quedan 1292. Se desaloja al final de `collectSamples()`, nunca antes, así que jamás tira algo que la propia llamada necesita. La verificación posterior corrigió la cota que se anunció: **no era «lo que registra el proyecto abierto»** sino «lo que registraba el último proyecto que se EXPORTÓ», porque el único barrido colgaba de `collectSamples()` — cerrar una canción y trabajar tres horas en otra sin exportar dejaba el audio decodificado de la primera entero en memoria. La medida sí era real; la frase, no. Desde que el barrido cuelga de `collectWorkletSamples` (abrir un `.orbit`, plantilla, restaurar versión, recuperar autosave) la cota anunciada es por fin la de verdad | v3.7 |
| **Las dependencias de los hooks, revisadas una a una**: cinco de los once avisos sobraban a propósito y ahora dicen por qué; los otros seis se completaron. La verificación posterior comprobó que **ninguno de los seis podía producir un fallo visible** —la dependencia que faltaba ya estaba cubierta transitivamente por otra de la misma lista, o apuntaba a una identidad estable—, así que la afirmación original («eran bugs reales») no se sostiene; el cambio sí vale, porque deja de depender de una cobertura accidental. `exhaustive-deps` pasa de aviso a error: uno nuevo rompe la CI en vez de sumarse a la pila | v3.7 |
| **El pluck recupera su punch**: `filters.ts` suavizaba los coeficientes en 5 ms para matar el zipper al automatizar, pero eso asume que `set()` se llama por bloque — y `SynthVoice`, `PrismaVoice` y `AutofilterUnit` lo llamaban por MUESTRA, con una envolvente que ya es continua, apilando un one-pole de 5 ms sobre un ataque de 5 ms. Ahora el filtro sabe de qué clase es su llamante (`CoefSource`, en el constructor: en modo `per-sample` el coeficiente de suavizado es 0, o sea la línea que desliza pasa a ser la identidad, sin ramas nuevas en el bucle de audio). Medido: 90 % del brillo a **3,42 ms en vez de 6,37**, que es exactamente la referencia sin suavizar — el one-pole no se acelera, desaparece. Y +11 dB de RMS sobre 3 kHz en los primeros 4 ms | v3.8 |
| **El fixture 25, y la lección de por qué hizo falta**: quitarle el suavizado a esos tres llamantes bajó la mordida del golden sobre `COEF_SMOOTH_SECONDS` de 7 fixtures a 3 — consecuencia lógica, no descuido. Pero dejaba sin fijar la pieza que ahora impide que vuelva el zipper (el autofiltro deslizando él mismo el corte que le llega por bloque): **ningún fixture automatizaba un parámetro del autofiltro**, así que el banco pasaba igual con esa pieza y sin ella. `fx-autofilter-sweep` existe para eso, y se comprobó quitándole la pieza a propósito: 35 medidas fuera de tolerancia y **14,694 dB** en la banda alta. Queda escrito en `docs/GOLDEN.md`: si una fila de la tabla de mordida baja, preguntate si bajó porque el motor mejoró o porque el banco dejó de mirar | v3.8 |
| **`--accept` deja de tragarse el flag siguiente como motivo**: `npm run golden:update -- --accept --force` guardaba `"--force"` como explicación del cambio de sonido **y** activaba el bypass de la guarda de arquitectura, de un solo gesto — el que cualquiera teclea sin pensar. Un espacio en blanco pasaba igual. La regla («un flag nunca es el valor de otro flag») vive en `tools/qa/cli-args.ts` y no dentro de cada script, porque el fallo era de la clase y no del archivo | v3.8 |
| **Cinco tests dejan de medir la CPU de la máquina en vez del código**: dos ya se habían visto en rojo en la CI de Windows. Los dos del cuelgue de plugin —estructuralmente idénticos, y la tarjeta solo fichaba uno— pasan a un reloj falso compartido (`fake-clock-pump.ts`), así que cuántas vueltas hacen falta para cruzar el deadline es un cálculo y no una medida de cuánta CPU había libre; se conserva la espera real entre vueltas, que es lo que demuestra que el hilo del host no se bloqueó. Donde no hay reloj que fingir, margen **medido** tras reproducir el fallo. Comprobado con veinte procesos quemando CPU en paralelo | v3.8 |
| **La CI de Windows llevaba seis pushes en rojo, y la release de la v3.8.0 se cortó encima**: un test que lee código fuente afirmaba tres líneas con `
` literales, y el checkout de Windows (`core.autocrlf=true`, el del runner `windows-latest`) entrega el archivo con CRLF — ubuntu verde y windows rojo en la misma corrida. Reproducido pasando la fuente a CRLF a propósito: la lectura de antes contesta `false` a esa aserción, la normalizada `true`. Arreglado por la clase: los tests que leen fuente entran por `read-source.ts`, que normaliza al leer — nueve en ese commit, once al cerrar la ronda, y los dos nuevos ya nacieron usándolo. No con un `.gitattributes` con `eol=lf`, y el motivo se midió: `git ls-files --eol` da 584 archivos de texto y los 584 en LF, así que no hay nada que renormalizar — lo que hay son tests que afirman sobre bytes que decide el checkout | v3.9 |
| **El grafo de dependencias entre paquetes se escribe una sola vez**. La regla dura 6 describía `ui→core,engine,collab` cuando el repo tenía **seis aristas** que eso prohíbe, repartidas en 16 archivos — ocho de ellos solo para el paquete que la regla ni nombraba. Se reescribió la regla, no el código, porque `ARCHITECTURE.md` ya describía en prosa esas aristas: la lista era de la v0.1 y nadie la remidió. Ahora vive en `tools/eslint/package-graph.json` y la leen dos — la regla `orbit/package-boundaries` que la hace cumplir, y un test que falla si CLAUDE.md o ARCHITECTURE.md dejan de coincidir con ella o con los exports reales de `core`. Con dos matices que antes eran comentarios sueltos: `engine` usa de `core` el MODELO y no el estado (cero `ProjectStore`/`applyCommand` en `engine/src`), y `ui` es el renderer, así que nada de subrutas `node/` aunque la arista esté permitida | v3.9 |
| **La regla dura 4 vuelve a describir el repo, y hay quien la haga cumplir en los `.css`**: veintitantos literales y cinco `var(--x, #hex)` con fallback salen a token (16 nuevos). Las tres paletas por editor se quedan locales y literales por un motivo técnico, no por comodidad — pintan en `<canvas>` y leen el color con `getComputedStyle()`, que NO resuelve una custom property que referencia otra (`--b: var(--a)` vuelve como el texto `"var(--a)"`). `orbit/no-hardcoded-colors` pasa a mirar también los `.ts`, y los `.css` los cubre `npm run lint:css`. Comprobado que no cambia el aspecto: muestreados los valores computados en los tres temas, cada token resuelve al literal que sustituyó | v3.9 |
| **Las tres cachés de audio del hilo de UI comparten política**: cada una se acota por su CONJUNTO VIVO, que lo define su consumidor. La del render y la de picos sirven al proyecto entero, así que un tope de entradas ahí sería dañino; la del editor sirve a UN sample y es la única a la que el barrido por proyecto no acota —los cinco samples de cinco Normalizar siguen registrados—, así que solo ella lleva tope de recencia. Y el barrido dejó de colgar de `collectSamples()`: cuelga de `collectWorkletSamples`, o sea de las cuatro puertas que reemplazan el proyecto. Medido: cinco Normalizar pasan de 6 entradas / 2.304.000 B a 2 / 768.000 B; a escala real, de ~69 MB retenidos a ~23 de techo | v3.9 |
| **El scope de la vista de instrumento mide el canal, no el máster**: `ChannelRack` pasaba `channel.mixerTrack` crudo, y un canal sin pista propia dentro de una carpeta con bus compila en el bus del grupo. Se arregló usando `trackOfChannel`, que ya existía en `core` y ya usaban MixTab, `run-export` y el executor del bridge — la quinta vez que aparece la misma pregunta y la primera en que no se escribe otra copia | v3.9 |
| **Un arrastre del deslizador de ganancia de entrada deja UN undo, no ochenta**: `patchInputRoute` despachaba sin `mergeKey`. El arreglo va en el envoltorio y la clave se deriva del propio patch (`route:${id}:${campo}`), así que cubre los tres llamantes y el próximo campo de `InputRoute` no reabre el bug. El campo va dentro de la clave a propósito: sin él, un arrastre que termina mientras se tipea el nombre se fundiría con él y un solo deshacer se llevaría el cambio de nombre | v3.9 |
| **`npm run listen:kit`**: el material que hay que ESCUCHAR, renderizado de los mismos fixtures del golden — así lo que se oye es exactamente lo que el banco fija. 25 s de cola añadida DETRÁS del contenido del fixture, o sea archivos de ~26,7 s y no de 25 (el golden corta a 1-3 para caber en la CI, y un piso de continua no se oye en dos segundos), y 24 bits (a 16 el contenedor esconde residuos que sí se oyen), más un índice que dice qué buscar en cada archivo y la referencia de libopus al mismo bitrate. Las medidas del índice salen de decodificar el `.wav` que hay al lado, no del buffer float que devolvió el motor: en la cola de la reverb el motor deja −364 dBFS de continua, que no alcanza el medio escalón de 24 bits (−144,5 dBFS), así que `encodeWav` la redondea a cero y en el archivo esos últimos 5 s son silencio digital exacto. Las dos cifras son legítimas y contestan preguntas distintas —qué hace el motor, qué va a oír quien abra el archivo—, así que donde discrepan el índice da las dos y dice cuál es cuál, con la del archivo delante; y no se pasa a float32 porque preservaría el número sin hacerlo audible: ningún convertidor baja de −144 dBFS. No renderiza el «antes» de un cambio de DSP: para eso tendría que editar `packages/engine/src`, y eso un comando no lo hace | v3.9 |
| **Un Ctrl+Z mientras normalizabas podía llevarse el audio recién creado.** Entre `engine.loadSample(nuevoId, …)` y el `registerSample` del dispatch hay una ventana en la que el sample no está sujeto por nada, y `useShortcuts` dispara el recolector justo en Ctrl+Z y Ctrl+Y. No duraba «el mismo tick» como se supuso: dentro hay dos `await` que ceden el hilo (`decodeAudioData` y `crypto.subtle.digest`). Estaba en CINCO sitios; el peor, `recorder.ts`, recorre todas las tomas con un guardado y un hash por toma y despacha una sola vez al final — cientos de ms con audio recién grabado colgando. Se sujeta con `withPinnedSample` (la API existía con cero llamantes) y, en el bucle, sosteniendo TODAS las tomas hasta el dispatch final: despachar por toma no elimina el pin, solo lo acorta, y rompería el historial y dónde caen las tomas | v3.10 |
| **El nombre de archivo era un borrado silencioso**: las ediciones se guardaban como `Edit HH.MM.SS.wav`, sin fecha, y `recording:save` pisa por nombre — la edición de hoy a las 14:03:22 se llevaba el audio de la de ayer a la misma hora, con el proyecto viejo apuntando ahí y un hash que ya no describía el contenido. El nombre pasa a derivarse del CONTENIDO, lo que además recorta el disco un 80 %: cinco Normalizar dejan 1 archivo / 8.640.044 B en vez de 5 / 43.200.220, porque normalizar deja el pico en 0,97 y de la segunda en adelante el WAV sale byte a byte idéntico. La política de borrado (anotar al escribir, barrer aparte, `discard` reversible y no `delete`, ramas archivadas conservadas vía `unreachableIds`) está escrita y probada; el barrido queda inerte hasta que exista `recording:discard` en el main | v3.10 |
| **Entrar en una sala de colaboración soltaba el audio del proyecto anterior**: `join()` y `replay()` hacen `replaceProject` y el único listener solo LLENABA — el comentario que había decía «aquí es donde hay que volver a llenarlo», y era literalmente cierto. Se barre DESPUÉS de subir, porque al revés un `unregisterSample` remoto que caiga en el `await` por sample dejaría uno recién subido sin barrer hasta el próximo reemplazo. Cerró además un agujero propio: `loadedIds` rancio dejaba el sample mudo sin aparecer como ausente | v3.10 |
| **El linter de fronteras tenía cuatro puertas traseras**, todas dormidas y todas reproducidas con `eslint --stdin`: `require()` no lo miraba ningún listener, `import()` dinámico esquivaba el chequeo de nombres porque `ImportExpression` no tiene `.specifiers`, un barril que reexportara su propio `node/` colaba `ws` y `node:http` al bundle del renderer, y los alias eran una cuarta escritura del grafo que nadie sincronizaba (ahora se derivan del `tsconfig`). Un caso de test por agujero, cada uno comprobado desactivando su arreglo | v3.10 |
| **Los dos linters de color eran ciegos a `RGB(` en mayúsculas**: no eran dos bugs sino una regex sin flag `i` copiada dos veces. El predicado vive ahora una sola vez en `tools/eslint/color-literal.js` y lo leen la regla de ESLint y el script de los `.css`, con 9 tests que antes no existían. De paso, el `#` percent-encoded de una data-URI ya se caza, y `url(#dead)` deja de ser un falso positivo | v3.10 |
| **La release dice en qué estado estaba la CI de su commit**. La v3.8.0 se cortó con `CI` en failure sobre su propio SHA y nadie se enteró. Se decidió ANUNCIAR y no bloquear: un tag y la CI son triggers independientes, así que esperar puede colgarse o pedir un timeout arbitrario — y esa es la puerta que alguien se salta con prisas en un hotfix. El estado va al resumen del job, a un `::warning::` y a la primera línea del cuerpo de la GitHub Release, y `npm run ci:status` lo contesta en una línea con el mismo comando que usa el workflow | v3.10 |
| **El pan de un send dejaba 40 filas de historial por arrastre**: su `patch()` nunca aceptó `mergeKey`, y uno de sus cinco controles es un `type="range"` de 40 pasos. La clave lleva origen, destino y campo. Y la pregunta de fondo —¿puede un deslizador que despacha olvidarse la clave?— se contesta con un inventario cerrado de los 8 `<input type="range">` del renderer que se rompe si aparece un noveno, en vez de un analizador que tendría que rastrear props entre archivos | v3.10 |
| **Los contadores de fugas se pueden mirar desde la consola**: `uiAudioCacheStats()` no tenía un solo llamante, y la instrucción que una tarjeta dejó escrita («llamalo desde la consola del renderer») no se podía ejecutar porque no estaba en `window`. Va al bloque de ganchos de QA solo-DEV que ya existía, juntando los cinco contadores de la ronda | v3.10 |
| **Los `package.json` eran la cuarta escritura del grafo de paquetes**, y también mentían: cinco unidades importaban `@orbit/*` sin declararlo. Funcionaba por accidente de la topología de npm workspaces, no porque estuviera declarado. Un cuarto assert del test del grafo lo exige por contención (no igualdad: una hoja puede declarar de más), anclando en posición de import para no caer en la trampa de los comentarios | v3.10 |
